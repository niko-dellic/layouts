...
export function fillTabPicker(
  ...
  input.type = 'search';
  input.placeholder = message(options, 'Search tabs\u2026');
  input.setAttribute('role', 'combobox');
  input.setAttribute('aria-label', message(options, 'Search tabs'));
  input.setAttribute('aria-expanded', 'true');
  input.setAttribute('aria-autocomplete', 'list');
  const list = el(doc, 'div', 'layouts-picker-list');
  list.id = `layouts-picker-${++sequence}`;
  list.setAttribute('role', 'listbox');
  list.setAttribute('aria-label', message(options, 'Available tabs'));
  ...
  function paint() {
    ...
    status.textContent = matches.length
      ? message(options, '{count} tab types', { count: matches.length })
      : message(options, 'No matching tabs');
  }
  function search() {
  ...
  const cancel = el(doc, 'button', 'layouts-button', message(options, 'Cancel'));
  cancel.type = 'button';
  cancel.onclick = () => scope.dispose();
  dialog.replaceChildren(input, list, status, ...(persistent ? [] : [cancel]));
  dialog.setAttribute('aria-label', message(options, 'Choose a tab'));
